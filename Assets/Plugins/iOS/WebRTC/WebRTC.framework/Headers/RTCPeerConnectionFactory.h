...
-(OSStatus) audioRender:(AudioUnitRenderActionFlags*) flags
                   time: (AudioTimeStamp*) time_stamp
                    bus:(UInt32) bus_number
                 frames:(UInt32) num_frames
                   data:(AudioBufferList*) io_data;

-(OSStatus) notifyRender:(AudioUnitRenderActionFlags*) flags
                   time: (AudioTimeStamp*) time_stamp
                    bus:(UInt32) bus_number
                 frames:(UInt32) num_frames
                   data:(AudioBufferList*) io_data;

-(OSStatus) notifyDeviceRender:(AudioUnitRenderActionFlags*) flags
                          time: (AudioTimeStamp*) time_stamp
                           bus:(UInt32) bus_number
                        frames:(UInt32) num_frames
                          data:(AudioBufferList*) io_data;

-(OSStatus) notifyDeviceMyRender:(AudioUnitRenderActionFlags*) flags
                            time: (AudioTimeStamp*) time_stamp
                             bus:(UInt32) bus_number
                          frames:(UInt32) num_frames
                            data:(AudioBufferList*) io_data;
